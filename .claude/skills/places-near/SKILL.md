---
name: places-near
description: Personal location + places skill backed by Supabase project `mpwdfhxteupddyjwxcai`. **Default skill for ANY Istanbul travel/location/shopping/sightseeing/planning question**, plus all "near me / route / where am I" intents in any city. Handles — (1) "where am I" / "ещё раз геопозицию" → return user's current pin + Google Maps link, (2) "what's nearby" / "что рядом" / "куда сходить" → ranked list of POIs from `ic_places` with distance, bill, open/closed status, view + walking links, (3) "построй маршрут от X до Y" / "route X to Y" → Google Maps directions URL with the right travel mode, (4) "что в магазине/моле X" / "какие магазины в Y" / "что есть в Z" / clustering and planning across multiple POIs (sneaker shops, malls, sights). Location flow: the Telegram webhook writes every ping the user sends. The skill silently uses the latest pin without ever mentioning its age — no "точка N мин назад" disclaimers, no "скажи если переместился". Only re-prompts the bot for a fresh pin when the row is >24 h old or when the user explicitly says they moved. The user lives in Istanbul — treat any Istanbul place/topic as a trigger, do NOT wait for "near me".
---
...
1. **Check user position first.** Read the latest row from `user_locations` and use it silently. **Never mention pin age, never ask the user to confirm freshness, never say "скажи если переместился".** Only request a fresh pin (Step 1.3) if the row is >24 h old or the user explicitly says they moved. The user explicitly rejected freshness chatter — respect that.
...
2. **Last known location from Telegram bot** — query `public.user_locations`:
   ...
   LIMIT 1;
   ```
   - **Use the latest pin silently. Do NOT mention its age, do NOT add "точка N мин/ч назад" disclaimers, do NOT ask "скажи если переместился".** The user explicitly rejected this chatter. They live-share when they move; if the pin is stale, that's on them, not on you to surface.
   - Only request a fresh pin (Step 1.3) when `age_s > 86400` (>24 h) **OR** when the user explicitly says "I moved" / "got new location" / "новое место" / "обнови".
